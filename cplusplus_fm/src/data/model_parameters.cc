...
Model::Model(const std::string& filename) {
  ...
  if (this->DeserializeFromTxt(filename) == false) {
    Color::print_error(
...
void Model::SerializeToTXT(const std::string& filename) {
  ...
  o_file << "param_num_w_:" << param_num_w_ << "\n";
  o_file << "param_num_v_:" << param_num_v_ << "\n";
  /*********************************************************
...
bool Model::DeserializeFromTxt(const std::string& filename) {
  ...
  if(not setStringFromString(i_file,"score_func_:",LINE_LENGTH,score_func_)){
    return false;
  }
  if(not setStringFromString(i_file,"loss_func_:",LINE_LENGTH,loss_func_)){
    return false;
  }
  if(not setIndextFromString(i_file,"num_feat_:",LINE_LENGTH,num_feat_)){
    return false;
  }
  if(not setIndextFromString(i_file,"num_field_:",LINE_LENGTH,num_field_)){
    return false;
  }
  if(not setIndextFromString(i_file,"num_K_:",LINE_LENGTH,num_K_)){
    return false;
  }
  if(not setIndextFromString(i_file,"aux_size_:",LINE_LENGTH,aux_size_)){
    return false;
  }
  if(not setIndextFromString(i_file,"param_num_w_:",LINE_LENGTH,param_num_w_)){
    return false;
  }
  if(not setIndextFromString(i_file,"param_num_v_:",LINE_LENGTH,param_num_v_)){
    return false;
  }
   ...
   *  Write linear and bias term                      *
   *********************************************************/
  if(not setFloattFromString(i_file,"bias:",LINE_LENGTH,param_b_[0])){
    return false;
  }
  // linear term
  index_t idx = 0;
  for (index_t i = 0; i < param_num_w_; i += aux_size_) {
    ...
    std::string key="i_"+idx_str+":";
    if(not setFloattFromString(i_file,key,LINE_LENGTH,param_w_[i])){
      return false;
    }
    idx++;
  }
