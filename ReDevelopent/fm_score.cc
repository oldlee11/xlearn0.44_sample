...
real_t FMScore::CalcScore(const SparseRow* row,
...
#else
        index_t aligned_k = model.GetNumK();
        index_t align0 = aligned_k;
#endif
...
#else
            real_t _XMMv = v1 * norm;                      // __m128 XMMv = _mm_set1_ps(v1*norm);
            for (index_t _d = 0; _d < align0; _d += 1) {   // for (index_t d = 0; d < aligned_k; d += kAlign) {
                real_t _XMMs = *(s + _d);                  //   __m128 XMMs = _mm_load_ps(s+d);
                real_t const _XMMw = *(w + _d);            //   __m128 const XMMw = _mm_load_ps(w+d);
                _XMMs += _XMMw * _XMMv;                    //   XMMs = _mm_add_ps(XMMs, _mm_mul_ps(XMMw, XMMv));
                *(s + _d) = _XMMs;                         //   _mm_store_ps(s+d, XMMs);
            }                                              // }
#endif
        }
#if USEORGCODE
        ...
        for (SparseRow::const_iterator iter = row->begin();
          ...
          _mm_store_ss(&t_all, XMMt);
            ...
            for (index_t _d = 0; _d < align0; _d += 1) {      // for (index_t d = 0; d < aligned_k; d += kAlign) {
                real_t _XMMs = *(s + _d);                     //     __m128 XMMs = _mm_load_ps(s+d);
                real_t _XMMw = *(w + _d);                     //     __m128 XMMw = _mm_load_ps(w+d);
                real_t _XMMwv = _XMMw * _XMMv;                //     __m128 XMMwv = _mm_mul_ps(XMMw, XMMv);
                _XMMt += (_XMMwv * (_XMMs - _XMMwv ));        //     XMMt = _mm_add_ps(XMMt,_mm_mul_ps(XMMwv, _mm_sub_ps(XMMs, XMMwv)));
            }                                                 // }
          }
          real_t t_all=_XMMt;                                 // real_t t_all; _mm_store_ss(&t_all, XMMt);
#endif
  t_all *= 0.5;
  t_all += t;
  return t_all;
...
void FMScore::calc_grad_sgd(const SparseRow* row,
   ...
   *********************************************************/
        ...
        index_t align0 = aligned_k;
        real_t _XMMpg  = pg;
        real_t _XMMlr  = learning_rate_;
        real_t _XMMlamb= regu_lambda_;
#endif
  std::vector<real_t> sv(aligned_k, 0);
  real_t* s = sv.data();
  for (SparseRow::const_iterator iter = row->begin();
  ...
  // To avoid unseen feature
    ...
    for(index_t d = 0; d < aligned_k; d += kAlign) {
      __m128 XMMs = _mm_load_ps(s+d);
      __m128 XMMw = _mm_load_ps(w+d);
      __m128 XMMg = _mm_add_ps(_mm_mul_ps(XMMlamb, XMMw), _mm_mul_ps(XMMpgv, _mm_sub_ps(XMMs,_mm_mul_ps(XMMw, XMMv))));
      XMMw = _mm_sub_ps(XMMw, _mm_mul_ps(XMMlr, XMMg));
      _mm_store_ps(w+d, XMMw);
    }
#else
      real_t _XMMv = v1*norm ;                                    // __m128 XMMv = _mm_set1_ps(v1*norm);
      real_t _XMMpgv = _XMMpg * _XMMv;                            // __m128 XMMpgv = _mm_mul_ps(XMMpg, XMMv);
      for (index_t _d = 0; _d < align0; _d += 1) {                // for (index_t d = 0; d < aligned_k; d += kAlign) {
          real_t _XMMs = *(s+ _d);                                //    __m128 XMMs = _mm_load_ps(s+d);
          real_t _XMMw = *(w+ _d);                                //    __m128 XMMw = _mm_load_ps(w+d);
          real_t _XMMg = (_XMMlamb * _XMMw)+(_XMMpgv * (_XMMs - (_XMMw * _XMMv)));   //    __m128 XMMg = _mm_add_ps(_mm_mul_ps(XMMlamb, XMMw), _mm_mul_ps(XMMpgv, _mm_sub_ps(XMMs,_mm_mul_ps(XMMw, XMMv))));
          _XMMw -= (_XMMlr * _XMMg);                              //    XMMw = _mm_sub_ps(XMMw, _mm_mul_ps(XMMlr, XMMg));
          *(w+ _d) = _XMMw;                                       //    _mm_store_ps(w+d, XMMw);
      }                                                           // }
#endif
  }
}
