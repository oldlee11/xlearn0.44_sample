...
namespace youtubDnn {
    ...
    void Solver::init_train() {
        ...
        if (! hyper_param_.init_embedding_file.empty()){
            if (model_->DeserializeEmbeddingFromTxt(hyper_param_.init_embedding_file) == false) {
                std::cout<< "Cannot Load init embedding file from the file:" << hyper_param_.init_embedding_file.c_str() <<"\n";
                exit(0);
            }
        }
    ...
    void Solver::show_average_metric(){
        std::cout<<"epoch"<<"(%)\t|\t"<<"tr_loss"<<"\t|\t"<<"te_loss"<<"\t|\t"<<"te_metric"<<"\t|\t"<<"time_cost"<<"\t|\t"<<"best_epoch"<<"\n";
    }
    void Solver::show_train_info(real_t tr_loss,
        ...
        std::cout<<epoch<<"("<< static_cast<int>(epoch*1.0/total_epochs*100) <<")\t|\t"<<tr_loss<<"\t|\t"<<te_loss<<"\t|\t"<<te_metric<<"\t|\t"<<time_cost<<"\t|\t"<<best_epoch<<"\n";
    }
    ...
    void Solver::Train() {
        int    best_epoch=0;
        real_t best_metric_val=0.0;
        int stop_window_tmp=0;
        ...
        for (int n = 1; n <= hyper_param_.num_epoch; ++n) {
            ...
            if(n==1){
                // copy current para to best para
                model_->SetBestModel();
                best_epoch=n;
                best_metric_val=te_info.metric_val;
            }else{
                if(te_info.metric_val > best_metric_val){
                    stop_window_tmp=0;
                    // copy current para to best para
                    model_->SetBestModel();
                    best_epoch=n;
            ...
            if((stop_window_tmp>=hyper_param_.stop_window)&(hyper_param_.early_stop)){
                break;
            }

        }
        // copy best para to current para
        model_->Shrink();
        std::cout<<"the best param is epoch:"<<best_epoch<<"\n";
    }
