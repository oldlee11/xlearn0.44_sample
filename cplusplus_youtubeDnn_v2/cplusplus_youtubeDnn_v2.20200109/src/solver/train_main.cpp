    ...
    for (int i = 1; i < argc; i++){
        ...
        if((i%2==0)&(i>=2)){
            ...
            if(arg_name=="-tr"){
                ...
                hyper_param.num_K=std::stof(arg_value_str);      // 100
                std::cout<<"hyper_param.num_K:"<< hyper_param.num_K<<"\n";

            }else if(arg_name=="-init_embedding_file"){
                // "/data/ming.li/workspace/TrainModel/api/fm/demo/cplusplus_youtubeDnn_withoutuser/init.modle.youtubeDnn.withoutuser.txt";
                hyper_param.init_embedding_file=arg_value_str;
                std::cout<<"hyper_param.init_embedding_file:"<< hyper_param.init_embedding_file<<"\n";
            ...
            }else if(arg_name=="-earlystop") {
                hyper_param.early_stop = (arg_value_str == "true");  // true
                std::cout << "hyper_param.early_stop:" << hyper_param.early_stop << "\n";

            }else if(arg_name=="-stopwindow"){
                hyper_param.stop_window = std::stoi(arg_value_str);   // 2
                std::cout << "hyper_param.stop_window:" << hyper_param.stop_window << "\n";
