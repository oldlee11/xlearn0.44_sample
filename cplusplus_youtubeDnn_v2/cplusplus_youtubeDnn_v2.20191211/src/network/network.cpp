...
namespace youtubDnn {
    real_t Network::CalcScore(
        ...
        for (index_t layer_j = 0; layer_j < num_fullLayer; ++layer_j) {
            ...
            index_t output_num=model.GetNum_midScore_fulllayer(layer_j);
            // w*x
...
// Calculate gradient and update current modelParameter parameters.
    void Network::CalcGrad(
            const SparseRow* row,
            Model& model,
            index_t thread_i,
            real_t pg,
...
// Calculate gradient using sgd
// embedding is not trainable
    void Network::calc_grad_sgd(
            const SparseRow* row,
            Model& model,
            index_t thread_i,
            real_t pg,
        ...
        for(index_t _d = 0; _d < aligned_k; _d += kAlign) {
            ...
            g_tmp[_d] = pg *  *(modelParameter.GetmidScore_fulllayer(thread_i,num_fullLayer-1)+ _d);
            // calc the g for fulllayer(laster):element_mean[aligned_k]
            *(modelParameter.GetmidScore_fulllayer(thread_i,num_fullLayer-1)+ _d) = pg *  *(modelParameter.GetmidScore_TargitRidEmbedding(thread_i)+ _d);
            *(modelParameter.GetmidScore_TargitRidEmbedding(thread_i)+ _d)        = g_tmp[_d];
        }
         * */
        for(index_t _d = 0; _d < aligned_k; _d += kAlign) {
        ...
        for (int layer_j = num_fullLayer-1; layer_j >= 0; --layer_j) { // layer_j = 3,2,1,0
            if(layer_j!=0){
                input_num = model.GetNum_midScore_fulllayer(layer_j-1);
            ...
            index_t pass_g_num = model.GetNum_midScore_fulllayer(layer_j);
            real_t* pass_g     = model.GetmidScore_fulllayer(thread_i,layer_j);   // 局部梯度(激活函数前的梯度)

            // calc w 局部梯度(激活函数前的梯度) g
            for(index_t in_i=0; in_i < input_num; in_i++){
                real_t* w   = model.GetFulllayer_w(layer_j,in_i);
                real_t in_v = *(input+in_i);
                // calc weight change
                ...
                for(index_t out_i=0; out_i < pass_g_num; out_i++){
                    *(w_change+out_i) += learning_rate_ * in_v * *(pass_g+out_i);  // 累加w的变化量
                }
            ...
            // calc bias 局部梯度(激活函数前的梯度) g
            real_t* b_change = model.GetFulllayer_b_change(thread_i,layer_j);
            // calc b change
            for(index_t out_i=0; out_i < pass_g_num; out_i++){
                *(b_change+out_i) += learning_rate_  * *(pass_g+out_i);  // 累加w的变化量
            }
        }
        *(model.GetFulllayer_change_num()+thread_i) = (*(model.GetFulllayer_change_num()+thread_i))+1.0;
    }

/* 把多次累计的w变化量一次性更新,并清空   -- one Thread
void youtubeDnn_network::UpDate(
        youtubeDnn_Model& modelParameter,
        index_t thread_i
        ){
    index_t num_fullLayer = modelParameter.GetNumFullLayerCell();
    index_t input_num = 0;
    real_t change_num = *(modelParameter.GetFulllayer_change_num()+thread_i);
    if(change_num > 0.9){
        for (int layer_j = num_fullLayer-1; layer_j >= 0; --layer_j) { // layer_j = 3,2,1,0
            // weights
            if(layer_j!=0){
                input_num = modelParameter.GetNum_midScore_fulllayer(thread_i,layer_j-1);
            }else{
                input_num = modelParameter.GetNum_midScore_OthersEmbedding(thread_i);
            }
            index_t pass_g_num = modelParameter.GetNum_midScore_fulllayer(thread_i,layer_j);
            for(index_t in_i=0; in_i < input_num; in_i++){
                real_t* w   = modelParameter.GetFulllayer_w(layer_j,in_i);
                real_t* w_change = modelParameter.GetFulllayer_w_change(thread_i,layer_j,in_i);
                for(index_t out_i=0; out_i < pass_g_num; out_i++){
                    *(w+out_i) -= (*(w_change+out_i))/change_num;
                    *(w_change+out_i)=0.0;// reset
                }
            }
            // bias
            real_t* b   = modelParameter.GetFulllayer_b(layer_j);
            real_t* b_change = modelParameter.GetFulllayer_b_change(thread_i,layer_j);
            for(index_t out_i=0; out_i < pass_g_num; out_i++){
                *(b+out_i) -= (*(b_change+out_i))/change_num;
                *(b_change+out_i) =0.0;// reset
            }
        }
        *(modelParameter.GetFulllayer_change_num()+thread_i)=0.0; //reset
    }
}*/

// 把多次累计的w变化量一次性更新,并清空  --- AllThreads
    void Network::UpDate_AllThreads(Model& model){
        index_t num_fullLayer = model.GetNumFullLayerCell();
        index_t input_num = 0;
        real_t change_num_total =0.0;
        for(index_t thread_i=0; thread_i< model.GetthreadNumber(); thread_i++){
            change_num_total += (*(model.GetFulllayer_change_num()+thread_i));
        }
        // for (int layer_j = num_fullLayer-1; layer_j >= 0; --layer_j) { // layer_j = 3,2,1,0
        for (index_t layer_j = 0; layer_j < num_fullLayer; layer_j++) {
            // nums
            if(layer_j!=0){
                ...
                input_num = model.GetNum_midScore_OthersEmbedding(0);
            }
            index_t pass_g_num = model.GetNum_midScore_fulllayer(layer_j);
            // weights
            for(index_t in_i=0; in_i < input_num; in_i++){
                ...
                for(index_t thread_i=0; thread_i< model.GetthreadNumber(); thread_i++){
                    real_t* w_change = model.GetFulllayer_w_change(thread_i,layer_j,in_i);
                    for(index_t out_i=0; out_i < pass_g_num; out_i++){
                        *(w+out_i) -= (*(w_change+out_i))/change_num_total;
                        *(w_change+out_i)=0.0;// reset
                    }
                }
            }
            ...
            for(index_t thread_i=0; thread_i< model.GetthreadNumber(); thread_i++){
                real_t* b_change = model.GetFulllayer_b_change(thread_i,layer_j);
                for(index_t out_i=0; out_i < pass_g_num; out_i++){
                    *(b+out_i) -= (*(b_change+out_i))/change_num_total;
                    *(b_change+out_i) =0.0;// reset
                }
            }
        }
        for(index_t thread_i=0; thread_i< model.GetthreadNumber(); thread_i++) {
            *(model.GetFulllayer_change_num() + thread_i) = 0.0; //reset
        }
    }
