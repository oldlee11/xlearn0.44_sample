...
void Model::Initialize(const std::string& score_func,
  ...
  * midScore_threads_
  ...
  for (index_t threadi=0;threadi < threadNumber_; ++threadi){
      midScore_num_threads_[threadi][0]=aligned_k_;                                      //  TargitRidEmbedding
      midScore_num_threads_[threadi][1]=aligned_k_*(num_field_-1);                       //  OthersEmbedding
...
Model::Model(const std::string& filename,xLearn::HyperParam& hyper_param_,bool para_isjustloadembedding) {
...
bool Model::DeserializeFromTxt(const std::string& filename,
    ...
    for (index_t j = 0; j < num_feat_; ++j) {
        ...
        if(setnums==num_K_){
            em_w +=  setnums ;
            // skip the rest parameters
            index_t skip = aux_size_*aligned_k_-num_K_;
            em_w += skip;
        }else{
            return false;
        }
