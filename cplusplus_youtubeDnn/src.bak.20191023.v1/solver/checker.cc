...
bool Checker::check_train_options(HyperParam& hyper_param) {
      ...
      if (value < 0 || value > 5) {
        ...
        switch (value) {
          case 0:
            ...
            hyper_param.score_func = "youtubeDnn";
            break;
          case 1:
   ...
   *  Set default value                                    *
   *********************************************************/
  if (hyper_param.model_file.empty() ) {
    hyper_param.model_file = hyper_param.train_set_file + ".model";
  }
  if (hyper_param.txt_model_file.empty() || (hyper_param.txt_model_file.compare("none") == 0 ) ){
    hyper_param.txt_model_file=hyper_param.train_set_file + ".model.txt";
  }
