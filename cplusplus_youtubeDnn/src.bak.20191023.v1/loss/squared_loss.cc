...
static void sq_gradient_thread(const DMatrix* matrix,
                        Model* model,
                        index_t thread_i,
                        Score* score_func,
                        bool is_norm,
...
//                         master_thread
//------------------------------------------------------------------------------
void SquaredLoss::CalcGrad(const DMatrix* matrix,Model& model) {
  ...
  for (int i = 0; i < count; ++i) {
    size_t start_idx = getStart(row_len, count, i);
    size_t end_idx = getEnd(row_len, count, i);
